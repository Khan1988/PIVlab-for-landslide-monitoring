...
uimenu(m3,'Label','PIVlab settings','Callback',@curr_settings_Callback); %save setting
...
item=[0 item(2)+item(4) parentitem(3) 14];
...
item=[0 item(2)+item(4) parentitem(3)/2.5 2];
handles.iam_free = uicontrol(handles.uipanel5,'Style','pushbutton','String','I am Free','Units','characters', 'Fontunits','points','Position',[item(1)+margin+item(1) parentitem(4)-item(4)-margin-10.4 item(3)-margin*2 item(4)],'Callback', @iam_free_Callback,'Tag','roi_select','TooltipString','Select area of interest freely');

item=[0 item(2)+item(4) parentitem(3)/2.5 2];
handles.iam_free = uicontrol(handles.uipanel5,'Style','pushbutton','String','Clear Free','Units','characters', 'Fontunits','points','Position',[item(1)+margin+20.5 parentitem(4)-item(4)-margin-item(2)-1.2 item(3)-margin*2 item(4)],'Callback', @Clear_free_Callback,'Tag','roi_select','TooltipString','Clear area of interest freely');

item=[0 item(2)+item(4) parentitem(3)/2 2];
handles.iam_free = uicontrol(handles.uipanel5,'Style','pushbutton','String','Save ROI','Units','characters', 'Fontunits','points','Position',[item(1)+margin parentitem(4)-item(4)-margin-item(2)+3 item(3)-margin*2 item(4)],'Callback', @Save_ROI_Callback,'Tag','roi_select','TooltipString','Select area of interest freely');

item=[0 item(2)+item(4) parentitem(3)/2 2];
handles.iam_free = uicontrol(handles.uipanel5,'Style','pushbutton','String','Load ROI','Units','characters', 'Fontunits','points','Position',[item(1)+margin+17 parentitem(4)-item(4)-margin-item(2)+5 item(3)-margin*2 item(4)],'Callback', @Load_ROI_Callback,'Tag','roi_select','TooltipString','Clear area of interest freely');

item=[0 0 0 0];
parentitem=get(handles.multip02, 'Position');
item=[0 12+margin/2 parentitem(3) 20];
handles.uipanel6 = uipanel(handles.multip02, 'Units','characters', 'Position', [item(1)+margin parentitem(4)-item(4)-margin-item(2)-3 item(3)-margin*2 item(4)],'title','Object mask', 'Tag','uipanel6','fontweight','bold');
...
item=[0 item(2) parentitem(3)/2 1.5]; %% Finish button
handles.finishbutt = uicontrol(handles.multip05,'Style','pushbutton','String','Finish','Units','characters', 'Fontunits','points','Position',[item(1)+margin parentitem(4)-item(4)-margin-item(2) item(3)-margin*2 item(4)],'Callback', @finishbutt_Callback,'Tag','finishbutt','TooltipString','Finish analysis');
...
%%%%%%%%%%%%
item=[0 item(2)+item(4)+margin/4 parentitem(3)/3*2 1];
handles.Orig_Time_Text = uicontrol(handles.multip07,'Style','text','String','Real time','HorizontalAlignment','left','Units','characters', 'Fontunits','points','Position',[item(1)+margin parentitem(4)-item(4)-margin-item(2) item(3)-margin*2 item(4)],'Tag','Orig_Time_Text');

item=[parentitem(3)/3*2 item(2) parentitem(3)/3*1 1];
handles.Orig_Time_ChBox = uicontrol(handles.multip07,'Style','checkbox','Value',0,'Units','characters', 'Fontunits','points','Position',[item(1)+margin parentitem(4)-item(4)-margin-item(2) item(3)-margin*2 item(4)],'Callback',@Orig_Duration,'Tag','Orig_Time_ChBox','TooltipString','Use real snapshot times');
...
set(gca, 'ydir', 'reverse');
text (520,450,['version: ' retr('PIVver') sprintf('\n') retr('update_msg')], 'fontsize', 8,'fontangle','italic','horizontalalignment','right');
...
if handles.Orig_Time_ChBox.Value == 1
    try
        caluv_list = retr('caluv_list');
        put('caluv',caluv_list((currentframe + 1)/2));
    catch
        disp("Duration not calculated yet or calculating sum/mean. This message is caught at line")
        line_num=dbstack;
        disp(line_num(end).line)
    end
end
...
if size(filepath,1)>0
    ...
    if isempty(derived)==0   %derivatives were calculated
        ...
        if displaywhat==1 %vectors only
            ...
            currentimage = rgb2gray(currentimage);  %display in main window
        ...
        if vecskip==1
         ...
         %   scatter(x(typevector==0),y(typevector==0),'rx') %masked
            ...
            q2=quiver(x_reduced(typevector_reduced==2),y_reduced(typevector_reduced==2),...
                ...
                'Color', vectorcolorintp,'autoscale', 'off','linewidth',str2double(get(handles.vecwidth,'string')));
          %  scatter(x_reduced(typevector_reduced==0),y_reduced(typevector_reduced==0),'rx');%masked %To display masked region   
        end
        hold off;
        %streamlines:
...
% size(path,1)
if isequal(path,0) ==0     %checks if value of path is not zero, will returns 1 if it is
    if get(handles.zoomon,'Value')==1
        set(handles.zoomon,'Value',0);
        zoomon_Callback(handles.zoomon)
    end
    ...
    if sequencer==1      %sequencer = 1 mean images are A-B, A-B. sequencer = 0 mean A-B, B-C....
        for i=1:size(path,1)     %size(path,1) = number of files selected
            if path(i).isdir == 0 %remove directories from selection (ensures that the following 'if' is run only for files not directories)
                if exist('filepath','var')==0 %first loop
                    filepath{1,1}=path(i).name;
                else %for loop number > 1
                    filepath{size(filepath,1)+1,1}=path(i).name;   %size(filepath,1)+1   = number of filepaths already in filepath array + 1
                end
            end
        end
    else %sequencer=0 (A-B,B-C,....)
        for i=1:size(path,1)
            if path(i).isdir == 0 %remove directories from selection
                if exist('filepath','var')==0 %first loop
                    filepath{1,1}=path(i).name;   %A-
                else   %for loop number > 1
                    filepath{size(filepath,1)+1,1}=path(i).name; %-B
                    filepath{size(filepath,1)+1,1}=path(i).name; %-B
                end
            end
        end
    end
    if size(filepath,1) > 1      % if number of files selected > 1
        if mod(size(filepath,1),2)==1      % if number of files selected are Odd
            cutoff=size(filepath,1);       % cutoff = last file
            filepath(cutoff)=[];           % remove last file 
        end
       %filepath is a list of full paths, including the names of file
        filename=cell(1);          %creates a matrix of empty matrices of size 1x1 (In this case one empty matrix)
        for i=1:size(filepath,1)   % size of filepath will be size of images to be analysed
            if ispc==1             % ispc == 1 for windows
                zeichen=strfind(filepath{i,1},'\');  %return indices of '\'
            else
                zeichen=strfind(filepath{i,1},'/');   % for OS other than windows
            end
            currentpath=filepath{i,1};   %select ith full path
            if mod(i,2) == 1              %checks if current path is at even location
                filename{i,1}=['A: ' currentpath(zeichen(1,size(zeichen,2))+1:end)]; %removes the full path and leaves the name of file
            else    % for odd location
                filename{i,1}=['B: ' currentpath(zeichen(1,size(zeichen,2))+1:end)]; %removes the full path and leaves the name of file
            end
        end
        %extract path:
        
        %%pathname = path to last used directory
        %%filename = (A:first filename-B:second filename,....)
        %%filepath = path of all selected files including names of files
        pathname=currentpath(1:zeichen(1,size(zeichen,2))-1);  %path location of the directory where images are
        put('pathname',pathname); %So the GUI remember the last used path
        put ('filename',filename); %only for displaying (A:filename-B:filename,....)
        put ('filepath',filepath); %full path and filename for analyses
        put('path',path);
        sliderrange                                           %% Display names of files plus a slider to navigate
...
if handles.Orig_Time_ChBox.Value == 1
    try
        caluv_list = retr('caluv_list');
        put('caluv',caluv_list(currentframe))
    catch
        disp("catched on ")
        line_num=dbstack;
        disp(line_num(end).line)
...
%%%%%% I am free %%%%%%%%
function iam_free_Callback(~, ~, ~)
filepath=retr('filepath');
handles=gethand;
h = retr('h');
freehand = retr('freehand');
if size(filepath,1) > 1
    h{end+1} = drawfreehand();
    H = h{end};
    if freehand == false
        freehand = ~H.createMask();
    else
        freehand = freehand & ~H.createMask();
    end
    cols_with_1_nonzeros = find(sum(~freehand~=0)>0);
    rows_with_1_nonzeros = find(sum(~freehand'~=0)>0);
    startx=cols_with_1_nonzeros(1,1);
    endx=cols_with_1_nonzeros(1,end);
    starty=rows_with_1_nonzeros(1,1);
    endy=rows_with_1_nonzeros(1,end);
%     F=imcrop(I,[startx starty endx-startx endy-starty]);
    roirect=[startx starty endx-startx endy-starty];
    polymask = imcrop(freehand,roirect);
    put('roirect',roirect);
    put('polymask',polymask);
    put ('freehand',freehand);
    put ('h',h);
    
    clear cols_with_1_nonzeros rows_with_1_nonzeros startx endx starty endy
    toolsavailable(1);
end


function Clear_free_Callback(~, ~, ~)
% filepath=retr('filepath');
handles=gethand;
freehand=retr('freehand');
h = retr('h');
...
% delete freehand roi on clicking clear roi % 
ch = get(gca,'Children');
for i=1:1:length(ch)
    if strcmp(ch(i).Tag,'roi')
        delete(ch(i))
    end
end
clear_roi_Callback;
...
function Save_ROI_Callback(~,~,~)
filepath=retr('filepath');
handles=gethand;
if size(filepath,1) > 1 %did the user load images?
    [ROIfile,ROIpath] = uiputfile('*.mat','Save PIVlab ROI','PIVlab_ROI.mat');
    if isequal(ROIfile,0) | isequal(ROIpath,0)
        ...
        roirect = retr('roirect');
        h = retr('h');
        polymask = retr('polymask')
        save(fullfile(ROIpath,ROIfile),'freehand','roirect','h','polymask');
    end
end

function Load_ROI_Callback(~,~,~)
filepath=retr('filepath');
handles=gethand;
if size(filepath,1) > 1 %did the user load images?
    [ROIfile,ROIpath] = uigetfile('*.mat','Load PIVlab ROI','PIVlab_ROI.mat');
    if isequal(ROIfile,0) | isequal(ROIpath,0)
        %do nothing
    else
        load(fullfile(ROIpath,ROIfile),'freehand','roirect','h','polymask');
        try
            put('freehand', freehand);
            put('roirect',roirect);
            put('h',h);
...
% R = retr('roirect');
% h = retr('h');
% save('C:\Users\Khan\Desktop\PIV Lab\A.mat','A','R','h')


% function Load_ROI_Callback(~,~,~)
% % load('A');
% % load('R');
% load('C:\Users\Khan\Desktop\PIV Lab\A.mat','A','R','h')
% put('freehand',A);
% put('roirect',R);
% put('h',h);
%%%%%%%%%%%%%%%%%%%%%%%%%
...
% to disply freehand roi after loading %
hold on
if ~isempty(h)
    green = cat(3, ones(size(freehand)),zeros(size(freehand)), zeros(size(freehand))); 
    for i=1:1:length(h)
        hold on 
        h=imshow(green)
        hold off
        set(h, 'AlphaData', ~freehand/30) 
        set(h, 'Tag', 'roi')
    end
end
    
%     for i=1:1:length(h)
%         x = h{i}.Position(:,1);y=h{i}.Position(:,2);
...
filepath=retr('filepath');
if numel(filepath)>1
    try  % Much faster than catch
        a = imfinfo(filepath{selected});
        size_img(1) = (a.Width/2);
        size_img(2) = (a.Height/2);
    catch
...
%     centre(1)= size(imread(filepath{selected}),1)/2; %y  % But why??
...
if ok==1
    ...
    pathname = retr('pathname');
    sequencer=retr('sequencer');
    updater = retr('updater');
    calxy = retr('calxy');
%     freehand = retr('freehand');
%     roirect = retr('roirect');
%     polymask = imcrop(freehand,roirect);
%     put('polymask',polymask);
    Num_images = size(filename,1);
    a=dir([pathname '/*.jpg']);
    out=size(a,1);
    %%% flags %%%%%
    denoise_flag = false;
    i1_flag = false;
    i2_flag = false;
    warn = false;   % flag to show that a first movement was detected (false = not detected)
    %%%%%%%%%%%%%%
    resultslist=cell(0); %clear old results
    toolsavailable(0);
    set (handles.cancelbutt, 'enable', 'on');
    set (handles.finishbutt, 'enable', 'on');
    ismean=retr('ismean');
    maskiererx=retr('maskiererx');
    maskierery=retr('maskierery');
    ...
    while 1
        ...
        if i==1
            tic
            %%%%%%%image allignment %%%%%%
            fixed = rgb2gray(imread(filepath{i}));
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        end
        cancel=retr('cancel');
        if isempty(cancel)==1 || cancel ~=1
            if (i1_flag == false && i2_flag == false)
                i1 = filepath{i};   %path 1
                i2 = filepath{i+2};   %path 2
            elseif (i1_flag == false && i2_flag == true)
                i1 = filepath{i};   %path 1
                i2 = filepath{i+3};   %path 2
            end
        image1=imread(i1);  %read image at path 1
        image2=imread(i2);  %read image at path 1
                
                
                
%        if isempty(cancel)==1 || cancel ~=1
%             if denoise_flag == false
%                 i1 = filepath{i};   %path 1
%             else
%                 i1 = filepath{i-2};
%             end
%                 i2 = filepath{i+1}; %path 2
%                 image1=imread(i1);  %read image at path 1
%                 image2=imread(i2);  %read image at path 1
            try
                crop1 = imcrop(image1,roirect);
                crop2 = imcrop(image2,roirect);
                [BW1,maskedImage1] = segmentImage(crop1);
                [BW2,maskedImage2] = segmentImage(crop2);
                Clouds = BW1 & BW2;
            catch
                Clouds = [];
            end
            % Image Registration via phase correlation
            image1 = imcrop(image1,roirect);
            image2 =  imcrop(image2,roirect);
            %split image in two
%             [r , c, t] = size(image1);
%             if rem(c,2) == 0
%                 c=c/2;
...
%                 r=r/2;
%            end
%             image1_1 = image1([1:r],[1:c],:);       % first split of image 1
%             image1_2 = image1([1:r],[c+1:end],:);       % first split of image 1
%             image1_3 = image1([r+1:end],[1:c],:);     % second split of image1
%             image1_4 = image1([r+1:end],[c+1:end],:);     % second split of image1
%             image2_1 = image2([1:r],[1:c],:);       % first split of image 1
%             image2_2 = image2([1:r],[c+1:end],:);       % first split of image 1
%             image2_3 = image2([r+1:end],[1:c],:);     % second split of image1
%             image2_4 = image2([r+1:end],[c+1:end],:);     % second split of image1
%             
%             aligned1 = registerp(image2_1,image1_1);
%             aligned2 = registerp(image2_2,image1_2);
%             aligned3 = registerp(image2_3,image1_3);
%             aligned4 = registerp(image2_4,image1_4);
%             
%             image2_1 = aligned1.RegisteredImage;
%             image2_2 = aligned2.RegisteredImage;
%             image2_3 = aligned3.RegisteredImage;
%             image2_4 = aligned4.RegisteredImage;
            
%             image2 = [image2_1,image2_2;image2_3,image2_4];

%             aligned1 =  registerp(image2_1,image1_1);
%             image2_1 = aligned1.RegisteredImage; %image2 will be grayscaled
%             aligned2 =  registerp(image2_2,image1_2);
%             image2_2 = aligned2.RegisteredImage; %image2 will be grayscaled
%             image2 = [image2_1,image2_2];          
%            
...
%             image1_1 = image1(:,[1:c],:);       % first split of image 1
%             image1_2 = image1(:,c+1:end,:);     % second split of image1
%             image2_1 = image2(:,[1:c],:);       % first split of image 2
%             image2_2 = image2(:,c+1:end,:);     % second split of image 2
%             aligned1 =  registerp(image2_1,image1_1);
%             image2_1 = aligned1.RegisteredImage; %image2 will be grayscaled
%             aligned2 =  registerp(image2_2,image1_2);
%             image2_2 = aligned2.RegisteredImage; %image2 will be grayscaled
%             image2 = [image2_1,image2_2];
            ...
            aligned = registerp(image2,image1);
            image2 = aligned.RegisteredImage; %image2 will be grayscaled
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            image1 = rgb2gray(image1);
            image2 = rgb2gray(image2);
            image1=uint8(image1); %
            image2=uint8(image2); % 
            % Image duration calculation%%%%%%%%%%%%%%%
            if (handles.Orig_Time_ChBox.Value == 1) 
             caluv_list = retr('caluv_list');
             Interval = GetInterval(i1,i2);
             caluv=calxy/(Interval/1000);
             caluv_list(end+1) = caluv;
             put('caluv_list',caluv_list)
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            if size(image1,3)>1
...
%             h = retr('h');
            ...
            elseif get(handles.fftmulti,'Value')==1
                ...
                [x, y, u, v, typevector] = piv_FFTmulti (image1,image2,interrogationarea, step, subpixfinder, mask, roirect,passes,int2,int3,int4,imdeform,repeat,mask_auto,do_pad,polymask);
                %u=real(u)
                %v=real(v)
            end
            ...
            if (i1_flag == false && i2_flag == false)
                ...
                u2 = u; v2 = v;
                [u2, v2] = denois_filter(x,y,u, v);
                i1_flag = false;
                i2_flag = false; 
                [u, v] = CF(x,y,u1,u2,v1,v2);
            end
...
%                 [u1, v1] = denois_filter(x,y,u, v);
%                 u3=u1;v3=v1;
%                 [u, v] = Cosine_Filter(x,y,u1,u2,u3,v1,v2,v3);
% %                 find_creep(u,v);
%                 u1 = u2; v1 = v2;
%             end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%            [sectioned_roi,sectioned_vecs] = gererate_sections(x,y,u,v,roirect,columns,rows);
%            warn = ShowTimeLapse(x,y,u,v,sectioned_roi,sectioned_vecs,filepath,i1,i2,warn);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            set(handles.totaltime,'string', ['Time left: ' sprintf('%2.2d', hrs) 'h ' sprintf('%2.2d', mins) 'm ' sprintf('%2.2d', secs) 's']);
                ...
                %%%% Update files%
        while updater == 1 && loops == size(filepath,1) - 1
            updates
            filepath=retr('filepath');
            filename=retr('filename');
            pathname = retr('pathname');
        ...
        finish = retr('finish');
        loops=loops+2;
        if updater == 0 && loops == size(filepath,1) - 1
            break
        end
        if finish == 1
            put('finish',0);
            set (handles.finishbutt, 'String', 'Finish');
...
if ok==1
    ...
    cancel = retr('cancel');
    if isempty(cancel)==1 || cancel ~=1
        %komplette resultslist vollschreiben!
...
function finishbutt_Callback(~, ~, ~)
handles=gethand;
put('finish',1);
set(handles.finishbutt, 'String', 'wait');
%  drawnow;
%  toolsavailable(1);
...
try
    ...
    put('caluv',caluv);
    %%%%%
    set(handles.Orig_Time_ChBox,'value',Orig_Time_ChBox);
    %%%%%%
    put('updater',updater);
...
Orig_Time_ChBox = get(handles.Orig_Time_ChBox,'value');
...
% Note:originally it was i=1:floor(size(filepath,1)/2) for some reason
    filtervectors(i)
...
s = regexprep(boxcontent, ',', '.');  %replaces (,) in boxcontent with (.)
s
set(who,'String',s);
...
function updates()
    path = retr('path');   %struct array
    pathname = retr('pathname');    %only the path to directory in use (excluding filename)
    filename=retr('filename');      % Array A:filename B:filename
    filepath=retr('filepath');
    sequencer = retr('sequencer');
    num_images = size(filepath,1);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    last_image = filepath{end};     % round braces to get the cell, curly braces to get whats inside cell 
    idx = strfind(last_image,"\");
    last_image = last_image(idx(end)+1:end);
    files_in_directory = dir(pathname);
    size_of_directory = size(files_in_directory,1);
    % index of last image
    for i = 1:size_of_directory
        if strcmp(files_in_directory(i).name,last_image);
            index=i;
        end
    end
    new_images = size_of_directory - index;
    if new_images>0
        if sequencer == 1  
            if new_images == 1   %cannot make a pair with 1 new image
                return
            else
                for i=index+1:size_of_directory   
                    if files_in_directory(i).isdir == 0   %new files are images and not directories
                        filepath{size(filepath,1)+1,1}=strcat(pathname,'\',files_in_directory(i).name);
                    end
                end
                if mod(new_images,2) == 1          %if number of new images are odd remove last one
                    cutoff=size(filepath,1);       % cutoff = last file
                    filepath(cutoff)=[];           % remove last file 
                end
            end
            
            for i=num_images:size(filepath,1)   % size of filepath will be size of images to be analysed
                if ispc==1             % ispc == 1 for windows
                    zeichen=strfind(filepath{i,1},'\');  %return indices of '\'
                else
                    zeichen=strfind(filepath{i,1},'/');   % for OS other than windows
            ...
            filepath{size(filepath,1)+1,1}=path(end).name;
            for i=index+1:size_of_directory   
                if files_in_directory(i).isdir == 0   %new files are images and not directories
                    filepath{size(filepath,1)+1,1}=strcat(pathname,'\',files_in_directory(i).name);
                    filepath{size(filepath,1)+1,1}=strcat(pathname,'\',files_in_directory(i).name);
...
%%%% New callbacks %%%%%
function Orig_Duration(~, ~, ~)
handles=gethand;
handles.Orig_Time_ChBox.Value
