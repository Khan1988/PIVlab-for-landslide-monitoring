function [xtable, ytable, utable, vtable, typevector] = piv_FFTmulti (image1,image2,interrogationarea, step, subpixfinder, mask_inpt, roi_inpt,passes,int2,int3,int4,imdeform,repeat,mask_auto,do_pad,polymask)
%profile on
%this funtion performs the  PIV analysis.
warning off %#ok<*WNOFF> %MATLAB:log:logOfZero
M=ones(size(image1,1),size(image1,2));
if numel(roi_inpt)>0
    xroi=roi_inpt(1);
    yroi=roi_inpt(2);
    widthroi=roi_inpt(3);
...
elseif (size(polymask,1)>0)
    mask = polymask;
%     for ff = 1:1:size(h,2);
%          m = ~h{ff}.createMask();
%          M = M & m;
%     end
%     mask = imcrop(M,roi_inpt);
%     mask = mask | ~Clouds;
%     mask(M)=1;
%         h{ff}.Position(end,:) = h{ff}.Position(1,:); % This line is required to close an open roi
%         h{ff}.Waypoints(end,:) = h{ff}.Waypoints(1,:);
%         mask{1} = [mask{1}; h{ff}.Position(:,1)];
%         mask{2} = [mask{2}; h{ff}.Position(:,2)];
%     end
else
    mask=zeros(size(image1_roi));
end
...
for multipass=1:passes-1
    ...
    if GUI_avail==1
        ...
        colorbar('off')
        %%% Michael Lims input %%%
%         utable_orig(utable_orig<0)=0;
%         vtable_orig(vtable_orig<0)=0;
        %%%%%%%%%%%%%%%%%%%%%%%%%%
